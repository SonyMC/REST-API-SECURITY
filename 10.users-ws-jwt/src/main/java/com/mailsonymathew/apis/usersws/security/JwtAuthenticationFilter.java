...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter { // Note :
	...
	// Note:We cannot define content values in application.properties file for hhis calss as follows as then we will have to annotate this calls with @Componentor @Service or @Repository to extract usign @Value.
	...
	// the JWT token from authentication token generated above
	// the authentication chain is automatically redirected to this function from
	// the above attemptAuthentication function
	@Override
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		String jwtToken = JWT
				...
				.withClaim("Roles",authUser.getAuthorities()  // add authorities as a claim
					.stream()  // Authorities will be a collection so have to stream each authority and add it to a collection separated by a comma
					.map(authority -> authority.getAuthority()) // get each authority
					.collect(Collectors.joining(",")))  // separate collection by a comma
