...
Cross-Origin Resource Sharing )is a mechanism to allow two different domains to talk to each other (by relaxing same-origin policy)
		                            ...
		                            http.
		                                 ...
		                                 // .authorizeHttpRequests() // authorize requests using antMatcher() methods  below. Authority is also called a Role & User is also known as a Principal
		                                  .authorizeRequests()  // Use .authorizeRequests() instead of authorizeHttpRequests() as we need to use 'access' method beow
										 ...
										 // .antMatchers("/v1/books/{bookId}").hasAnyAuthority("USER","ADMIN") // authorize GET request fro both USER and ADMIn authorities
		                                  /*
		                                   * Note : Spring Security automatically prefixes hasRole with 'ROLE' so there is no need to specify ROLE_USER or ROLE_ADMIN explicitly as we have given in DB tables.However if we explictilty prefix ROLE , Spring Security is inelligent enough not to prefix it again 
		                                   */
		                                 ...
		                                 // .antMatchers("/v1/books/{bookId}").access("hasRole('ROLE_USER') and hasAuthority('GET_BOOK')") // Refer AUTHORITIESV2 table
										 // .antMatchers("/v1/books").access("hasRole('ROLE_ADMIN') and hasAuthority('CREATE_BOOK')") // Refer AUTHORITIESV2 table        // authorize POST reques to create a Book only for ADMIN( the authority is defined in Authorites table for the username 'admin')
