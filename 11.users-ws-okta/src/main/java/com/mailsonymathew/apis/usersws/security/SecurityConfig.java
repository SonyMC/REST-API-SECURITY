...
Cross-Origin Resource Sharing )is a mechanism to allow two different domains to talk to each other (by relaxing same-origin policy)
		                            ...
		                            http.
		                                   ...
		                                   * For users-ws we will not need antMatchers
		                                   */
		                                 ...
		                                 .and()
		                               //  .addFilter(new JwtAuthenticationFilter(authenticationManager()))  // register our JwtAuthenticationFileter . We will be using Okta instead
		                              //   .addFilter(new JwtAuthorizationFilter(authenticationManager(), userDetailsServiceImpl))  // we do not require JwtAuthorizationFilter for users-ws as authorization will eb done by books-ws and authors-ws 
		                              ...
		                              // .sessionManagement() .sessionCreationPolicy(SessionCreationPolicy.STATELESS)  // disable session management for OAuth2
